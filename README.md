...
Pi Guardian is a handful of small, focused Python modules — no heavy framework
lock-in, no opaque background daemon:

| File | Role |
|---|---|
| **`dashboard/main.py`** | The **FastAPI web app** — serves the dashboard pages, the REST API and the live **WebSocket** feed, plus authentication, rate limiting and remote actions. This is what you run as a long-lived service. |
| **`collector.py`** | **Shared data layer.** Reads CPU / temperature / RAM / disk via `psutil` (with a `/proc` fallback) and pulls AdGuard Home stats. Both the dashboard and the monitor import it, so they always report the same numbers. |
| **`monitor.py`** | The **notifier**, run from cron. Collects the current state and pushes it out over **e-mail**, **Telegram**, **Healthchecks.io** and **Uptime Kuma**. Three modes: `report` (full status), `alert` (only when a threshold is breached) and `test` (print to terminal, send nothing). |
| **`monthly_report.py`** | Generates a polished **monthly PDF report** with ReportLab — system summary, AdGuard totals, events and logins — and e-mails it. Run from cron on the 1st. |
| **`demo_data.py`** | Synthetic-data provider used when `DEMO_MODE=1`, so every module above works with no Raspberry Pi and no external services. |

**Data flow:**

```
                       ┌── dashboard/main.py ──► live UI + REST API + WebSocket
collector.py ──────────┤
 (psutil + AdGuard)    ├── monitor.py (cron) ──► e-mail · Telegram · Healthchecks · Uptime Kuma
                       └── monthly_report.py (cron) ──► monthly PDF ──► e-mail
```

### 🔔 Notification channels (`monitor.py`)

Run from cron (`python monitor.py alert` / `report`). Every channel is optional —
leave its variables blank in `.env` and it is simply skipped.

| Channel | Configured via | Used for |
|---|---|---|
| 📧 **E-mail** (Gmail / SMTP) | `GMAIL_USER`, `GMAIL_APP_PASS`, `EMAIL_TO` | Status & alert e-mails + the monthly PDF |
| 💬 **Telegram** | `TG_BOT_TOKEN`, `TG_CHAT_ID` | Instant alert / report messages |
| 💓 **Healthchecks.io** | `HC_PING_URL` | Dead-man's-switch heartbeat (up / fail) |
| 📡 **Uptime Kuma** | `UPTIME_KUMA_PUSH_URL` | Push-monitor heartbeat |

> The dashboard also sends **registration approval** e-mails (approve/reject a new
> account straight from your inbox) when Gmail is configured.
